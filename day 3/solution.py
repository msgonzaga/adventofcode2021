...
def solution(input_data):
    ...
    for bit_idx in range(len(input_data[0])):
        oxygen_input = [d for d in oxygen_input if int(
            d[bit_idx]) == most_common[bit_idx]]
    ...
    for bit_idx in range(len(input_data[0])):
        co2_input = [d for d in co2_input if int(
            d[bit_idx]) == least_common[bit_idx]]
        if len(co2_input) == 1:
            break
    ...
    oxygen_num = sum(
        [int(oxygen_input[i])*2**i for i in range(len(input_data[0]))])
